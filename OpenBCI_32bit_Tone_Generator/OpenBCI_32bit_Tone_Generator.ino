 ...
 *
 *      This code will recieve a trigger signal on the serial port
 *      and insert auxiliary data into the data stream to mark the trigger.
 *      Controlling program should use '`' as the trigger.
 *      Board will insert 0x6220 in all three aux channels.
 *      0x6620 converts to pi when decoded as Gs, becaue OpenBCI_GUI
 *      expects to see accelerometer data in this position.
 *      Also, the Board will blink it's blue LED for 500mS.
 *
...
volatile boolean addAccelToSD = false;
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//  << TONE Business >>
int Speaker = 18;  // alias for pin D18
unsigned long toneStartTimer = 0;
//------------------------------------------------------------------------------
void setup(void) {

  Serial0.begin(115200);  // using hardware uart number 0
  pinMode(LED, OUTPUT);  digitalWrite(LED,LOW);    // blue LED
...
void loop() {
  ...
  if(is_running){
      ...
      toneStartTimer++;
      if(toneStartTimer%500 == 0){  // tone happens every this number of mS times 4
...
// STREAM DATA AND FILTER COMMANDS
      case 'b':  // stream data
        ...
        toneStartTimer = 1;       // don't hit the subject with a tone right out of the gate!
        break;
